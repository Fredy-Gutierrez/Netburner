...
TrainControl control = { {'0','4','0','4','2','0','2','2','\0'} , 0, {'0','5','0','4','2','0','2','2','\0'} , 0, 0 };
...
void overwriteTrainControl(){
	...
	if(fp){
		SDWrite(&control, sizeof(control), fp);//write train control structure
...
void readTrainControl(){
	...
	if(fp){
		SDRead( (void *)&control, sizeof(TrainControl), fp );//read train control structure
		CloseFile(fp);//close file
	}else{
...
bool useTrainDirectory(RequestType request, bool useTrainControl = true){
	...
	if(useTrainControl){
		if(request == ReadRequest || request == OverWriteRequest){
			memcpy(trainFolder, control.DayDirRead, sizeof(trainFolder));
		}else{
			...
			trainFolder[8] = '\0';
			int datesCom = compareDates(control.DayDirWrite, trainFolder);//COMPARE TWO (CHAR *) DATES TO KNOW WHICH IS HIGHEST
			if( datesCom < 0 ) {//IF ACTUAL CONTROL DAYWRITE IS LOWER THAN SYSTEMDATE
				memcpy(control.DayDirWrite, trainFolder, sizeof(trainFolder));//CHANGE THE DAYWRITE TO SYSTEMDATE
				control.TrainNumWrite = 0;//RESTART THE TRAIN NUMBER FOR NEW DAY
				...
				memcpy(trainFolder, control.DayDirWrite, sizeof(trainFolder));
			}
		}
...
void writeTrainIntoDB(){
	...
	if(fp){
		...
		if(userRW->userRequestFN != NULL){//if user want that the DBTrain call any function, it will call function
			userRW->userRequestFN( rv > 0 );//call the user function passing true or false depending the write result
		}
	}
...
void overWriteTrain(){
	...
	if(fp){
		...
		CloseFile(fp);//close train file
		if(userRW->userRequestFN != NULL){//if user want that the DBTrain call any function, it will call function
			userRW->userRequestFN( rv > 0 );//call the user function passing true or false depending the overwrite result
		}
	}
...
void readNextTrain(){
	...
	while(!lecture){//until a train is read or there is not trains in folder to read or if user is using the train control it will look for next train moving the day folder to next upto dayread equals daywrite
		...
		if(fp){
			...
			CloseFile(fp);//close train file
			if(userRW->userRequestFN != NULL){//if user want that the DBTrain call any function, it will call function
				userRW->userRequestFN( rv > 0 );//call the user function passing true or false depending the read result
			}
			...
			if(userRW->useTrainControl){//if user is using the train control
				if(compareDates(control.DayDirRead, control.DayDirWrite) < 0){//if day read is lower than day write, the day read directory has been completely read, so we must move to next directory
					...
					if(userRW->userRequestFN != NULL){//if user want that the DBTrain call any function, it will call function
						userRW->userRequestFN( false );//false because there are no train for the moment
					}
					lecture = true;//to close while
				}
			}else{//if user is not using the train control
				if(userRW->userRequestFN != NULL){//if user want that the DBTrain call any function, it will call function
					userRW->userRequestFN( false );
				}
				lecture = true;//to close while
			}
		}
			...
			case StDbWait:
				if(UserDBRequest != StNoRequest){
					switch(userRW->request){
						case ReadRequest:
							DBProcessState = StDbRead;
							break;
						case WriteRequest:
							DBProcessState = StDbWrite;
							break;
						case OverWriteRequest:
							DBProcessState = StDbOverWrite;
							break;
					}
				}
...
void InitDBTrain(){
	OSSimpleTaskCreatewName(DBTrainProcess, FREE1_PRIO, "TaskDBTrain");
}

/******************************************RETURN THE QUANTITY OF TRAINS THAT MUS BE SEND*******************************/
uint32_t getQtyTrainsToSend(){
	return control.TrainToSend;
}
