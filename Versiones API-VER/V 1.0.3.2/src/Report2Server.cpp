...
bool makeRequest(RequestType request){
	if(request == ReadRequest){
		...
		trainDBUserR.train = &bufferTrainRead;
		trainDBUserR.userRequestFN = trainFound;
		trainDBUserR.useTrainControl = true;
	}else if(request == OverWriteRequest){
		trainDBUserR.request = request;
		trainDBUserR.train = &bufferTrainRead;
		trainDBUserR.userRequestFN = NULL;
		trainDBUserR.useTrainControl = true;
	}else{
		return false;
	}
...
/*********************************FUNTION THAT USER WANT THE DBTRAIN CALL AFETER REQUEST**********************/
void trainFound(bool result){
	if(result){//true for there is a train, false for no train
		trainReq = StCompletedReq;//update the request state to say that there is a train to send
	}else{
		trainReq = StNoRequest;//update the request state to say that there is no train to send so we must wait
	}
}
		...
		switch(state){
			...
			case StWait:
				...
				if(++ticksToSendStatus >= 600 && state == StWait){		//600 FOR 5 minutes - 200 FOR 1 minutes
					...
					retries=0;
					state = StSendStatus;
				}else if(trainReq == StNoRequest){	//When there is not train in buffer
					if(++ticksToReq >= 240){//	and have passed 240 ticks	(2 minutes)
						state = StAskForTrain;//	moves to ask for train
						ticksToReq = 0;
					}
				}else if(trainReq == StCompletedReq && netAvailable && serverAvailable){	//There is a train to send and Ethernet connection and server are available
					ticksToReq = 0;
					retries=0;
					state = StSendTrain;
				}
			...
			case StAskForTrain:
				...
				if(makeRequest(ReadRequest)){
					trainReq = StInProgressReq;//request state is waiting the DB Response
			...
			case StAfterSend:
				...
				if(makeRequest(OverWriteRequest)){
					state = StWait;
					trainReq = StNoRequest;//Change the request state to say that there is not train to send
				}else{
					OSTimeDly(10);
				}
				break;
