...
void validateServerValues(){
	if(strcmp(oldServerConf.url.c_str(), newServerConf.url.c_str()) != 0 && strcmp(newServerConf.url.c_str(), "") != 0){
		setServerUrl(strdup(newServerConf.url.c_str())); }
	saveSettings();
}
...
void getOldServerValues(){
	oldServerConf.url = (NBString)getServerUrl();
}
void getOldInValues(){
