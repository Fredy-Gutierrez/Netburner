...
typeSegIdx segIdx[MAX_SEGS_TRAIN];
static int qtyAxles;
...
bool trainAxlesProfile(){
	...
	while(axleIt < qtyAxles){
		typeAxle *axle = &axlesTrain[axleIt];
		if(prevDir == 0){
			prevDir = axle->dir;
			...
			if(prevDir == axle->dir){
				...
				if(residual > 0){
					...
					int auxDir = axle->dir;
					int axCount = 0;
					for(int itAux = axleIt; itAux < qtyAxles && axCount < residual; itAux++){
						if(auxDir == axlesTrain[itAux].dir){
							axCount++;
						}else{
							if(axCount < residual){
								residual += axCount;
...
bool axlesDebugging(){
	...
	logg.newPrint("\n****************Printing %d axles***************", qtyAxles);
	typeAxle * axle;
	for(i = 0; i < MAX_AXLES_TRAIN && i < qtyAxles; i++){
		axle = (typeAxle *)axles.Get();
		memcpy(&axlesTrain[i], axle, sizeof(typeAxle));
		axle = &axlesTrain[i];
		printAxle(axle, i + 1);
		...
		if(lastaAxle){
			...
			if( (diff >= axDiffRange && axle->distance >= axDistRange) || (speedDiff > 5 && axle->distance >= axDistRange) || (diff >= axDiffRange && speedDiff > 5 && axle->distance > 0)){
				...
				if(qtyAxles == 4 && axle->distance >= 1500){
					result = false;
					logg.newPrint("		; BAD AXLE");
				}
			}
...
SDTrainUsers trainDBUserW = { WriteRequest, NULL, StNoRequest, true, true, {'\0'}, {'\0'}};
...
void createAndSaveTrain(Fifo *aarTags){
	...
	while(train){
		...
		if(trainDBUserW.stateRequest == StCompletedReq){
			logg.newPrint("\nTrain %d Stored in SD Card", trainNum++);
		}else{
			logg.newPrint("\nTrain %d could not be stored, check SD Card", trainNum++);
		}
