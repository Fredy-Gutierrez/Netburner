...
enum stTagProc {stProcIni, stProcWait, stProcDelayToCodeAar, stProcTrainData, stReportToServer};
...
typeAarDec * decEncHexToAar(typeHexTag * newTag){
	...
	if(newTag != NULL){
		...
		if(saveAarTag){
			...
			return newTagAar; // saves in fifo
		}else{
			free(newTagAar);
		}
	}
...
void processorFunction(void * taskData){
	while(1){
		...
		case stProcDelayToCodeAar:
			...
			stateTagProc = stProcTrainData;
			break;
		case stProcTrainData:
			fillDataTrain(&hexTags);
